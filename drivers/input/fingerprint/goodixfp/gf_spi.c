...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_EXIT:
		pr_warn("%s GF_IOC_EXIT\n", __func__);
		break;

	case GF_IOC_DISABLE_IRQ:
		pr_warn("%s GF_IOC_DISABEL_IRQ\n", __func__);
		gf_disable_irq(gf_dev);
		break;

	case GF_IOC_ENABLE_IRQ:
		pr_warn("%s GF_IOC_ENABLE_IRQ\n", __func__);
		gf_enable_irq(gf_dev);
		break;

	case GF_IOC_RESET:
		pr_warn("%s GF_IOC_RESET\n", __func__);
	...
	case GF_IOC_DISABLE_SPI_CLK:
		pr_warn("%s GF_IOC_DISABLE_SPI_CLK\n", __func__);
#ifdef AP_CONTROL_CLK
		gfspi_ioctl_clk_disable(gf_dev);
#else
		pr_warn("doesn't support control clock!\n");
#endif
		break;

	case GF_IOC_ENABLE_POWER:
		pr_warn("%s GF_IOC_ENABLE_POWER\n", __func__);
		gf_power_on(gf_dev);
		break;

	case GF_IOC_DISABLE_POWER:
		pr_warn("%s GF_IOC_DISABLE_POWER\n", __func__);
		gf_power_off(gf_dev);
		break;

	case GF_IOC_ENTER_SLEEP_MODE:
		pr_warn("%s GF_IOC_ENTER_SLEEP_MODE\n", __func__);
		break;

	case GF_IOC_GET_FW_INFO:
		pr_warn("%s GF_IOC_GET_FW_INFO\n", __func__);
		break;

	case GF_IOC_REMOVE:
		pr_warn("%s GF_IOC_REMOVE\n", __func__);
/* Huaqin modify for JD2020-210 fingerprint request gpio error by puqirui at 2019/02/25 start */
		gf_dev->users = 0;
/* Huaqin modify for JD2020-210 fingerprint request gpio error by puqirui at 2019/02/25 end */
		irq_cleanup(gf_dev);
		gf_cleanup(gf_dev);
		break;

	case GF_IOC_CHIP_INFO:
		pr_warn("%s GF_IOC_CHIP_INFO\n", __func__);
...
static int __init gf_init(void)
{
	...
	pr_warn("platform_driver_register(&gf_driver)\n");
