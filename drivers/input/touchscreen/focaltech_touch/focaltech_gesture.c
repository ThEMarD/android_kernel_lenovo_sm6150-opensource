...
*****************************************************************************/
#define KEY_GESTURE_U                           KEY_POWER //KEY_GESTURE_DC
...
int fts_gesture_suspend(struct i2c_client *client)
{
    ...
    for (i = 0; i < 5; i++) {
#if FTS_FOD_EN
        //fts_i2c_write_reg(client, FTS_REG_FOD_ADDRESS, FTS_REG_FOD_EN_VALUE);//finder down when screen off
#endif
        fts_i2c_write_reg(client, 0xd1, 0xff);
        fts_i2c_write_reg(client, 0xd2, 0xff);
        fts_i2c_write_reg(client, 0xd5, 0xff);
...
/* huaqin modify for gesture by limengxia at 2019/02/20 start */
    fts_gesture_data.mode = DISABLE;
/* huaqin modify for gesture by limengxia at 2019/02/20 end */
    fts_gesture_data.active = DISABLE;
