...
*****************************************************************************/
/* Huaqin add for JD2020-208 ito test by limengxia at 2019/1/15 start */
#define HWINFO_NAME		"tp_wake_switch"
/* Huaqin add for JD2020-208 ito test by limengxia at 2019/1/15 end */
...
/* Huaqin add for JD2020-209 TP info by limengxia at 2019/01/24  start */
...
#endif
/* Huaqin add for JD2020-209 TP info by limengxia at 2019/01/24  end */
...
#if FTS_FOD_EN
    fts_i2c_write_reg(client, FTS_REG_FOD_ADDRESS, FTS_REG_FOD_EN_VALUE);
#endif
#if FTS_GESTURE_EN
...
static int fts_power_source_init(struct fts_ts_data *data)
{
	...
	ret = regulator_set_load(data->vdd,15000);
        if (ret) {
...
int fts_test_node_init(struct platform_device *tpinfo_device)
{
    ...
    if (0 != err)
    {
        printk( "[zhengwufocal] [FTS]%s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
        return -EIO;
    }
    else
    {
        printk("[zhengwufocal] [FTS]%s() - sysfs_create_group() succeeded.", __func__);
    }
    return err;
}
...
static int fts_driver_load = 0;
int fts_get_driver_load(void)
{
	return fts_driver_load;
}
...
#if 0 //FTS_FOD_EN
	    if (fp_match == 1) {
			FTS_DEBUG("FOD fp match report FP_MATCH");
			input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, FP_MATCH);
		}
	    else {
			FTS_DEBUG("FOD fp not match report FP_NOT_MATCH");
			input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, FP_NOT_MATCH);
		}
#else
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    ret = fts_i2c_read_reg(data->client, FTS_REG_FOD_ADDRESS, &state);
    if ((ret < 0) || (state != FTS_REG_FOD_EN_VALUE)) {
        FTS_DEBUG("FOD not enable, don't process FOD state = %d", state);
    } else if (state == FTS_REG_FOD_EN_VALUE) {
        ret = fts_i2c_read_reg(data->client, FTS_REG_GESTURE_OUTPUT_ADDRESS, &state);
        //FTS_DEBUG("[FOD]Ret = %d,Gesture ID =%d", ret, state);
	if (state == 38) {
		//FTS_DEBUG("FOD fp match");
		fp_match = 1;
	}
	else {
		//FTS_DEBUG("FOD fp not match");
		fp_match = 0;
	}
    }
#endif
...
#endif
/*huaqin modify for JD2020-6 finger down by limengxia at 2019/1/25 start*/
    data->point_num = 0;
    data->touch_point = 0;
	data->finger_down = 0;
	...
	data->finger_down = buf[1] & 0x0F;
	//FTS_INFO("finger down buf[1]: %x,data->finger_down=%d",buf[1],data->finger_down);
	if(data->finger_down == 6){
		hq_finger_down = 1;
	}else{
		hq_finger_down = 0;
	}
	//FTS_INFO("finger down : %d",hq_finger_down);
/*huaqin modify for JD2020-6 finger down by limengxia at 2019/1/25 start*/
...
#endif
#if FTS_FOD_EN
    fts_i2c_write_reg(ts_data->client, FTS_REG_FOD_ADDRESS, FTS_REG_FOD_EN_VALUE);
#endif
/*huaqin modify for bring up by limengxia at 2019/1/22 start*/
#if defined(CONFIG_DRM)
...
#if FTS_POWER_SOURCE_CUST_EN
/* Huaqin add for Power consumption by limengxia at 2019/2/22 start */
/* TP enter sleep mode */
    ret = fts_i2c_write_reg(ts_data->client, FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP_VALUE);
    if (ret < 0)
        FTS_ERROR("set TP to sleep mode fail, ret=%d", ret);
/* Huaqin add for Power consumption by limengxia at 2019/2/22 end */
    fts_release_all_finger();
